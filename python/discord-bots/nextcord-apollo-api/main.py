...
if os.getenv('BOT_TOKEN') == "TOKENGOESHERE" or os.getenv(
        'PANEL_KEY') == "ptlc_LSEiZPfRJvw3oMzbmC4XEp8gt3nw4ngi5Mdb9P8VXr7":
...
async def power(interaction,
    ...
    embed = nextcord.Embed(
        title="Success",
        description=
        f":white_check_mark: Successfully sent power action `{power_action}` to server `{server_id}`.",
        color=0x00ff04)
    await interaction.response.send_message(embed=embed)
...
async def command(
    ...
    embed = nextcord.Embed(
        title="Success",
        description=
        f":white_check_mark: Successfully sent command `{command}` to server `{server_id}`.",
        color=0x00ff04)
    await interaction.response.send_message(embed=embed)
